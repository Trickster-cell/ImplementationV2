...
class SSA(nn.Module):
    def __init__(self, d_model, num_heads): #dmodel will be embed dimensions
        ...
        self.q_lif  = snn.Leaky(beta = beta, spike_grad=surrogate_grad, output=True)

        self.W_k = nn.Conv1d(d_model, d_model, kernel_size=1, stride=1,bias=False)
        self.k_lif  = snn.Leaky(beta = beta, spike_grad=surrogate_grad, output=True)

        self.W_v = nn.Conv1d(d_model, d_model, kernel_size=1, stride=1,bias=False)
        self.v_lif  = snn.Leaky(beta = beta, spike_grad=surrogate_grad, output=True)

        # Linear transformation for output
        self.W_o = nn.Conv1d(d_model, d_model, kernel_size=1, stride=1)
        self.o_lif = snn.Leaky(beta = beta, threshold=0.5, spike_grad=surrogate_grad, output=True)
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        Q = Q.view(Q.size(0), -1, self.num_heads, self.head_dim).contiguous().transpose(1, 2)
        K = K.view(K.size(0), -1, self.num_heads, self.head_dim).contiguous().transpose(1, 2)
        V = V.view(V.size(0), -1, self.num_heads, self.head_dim).contiguous().transpose(1, 2)
