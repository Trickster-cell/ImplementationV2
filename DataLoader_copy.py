...
class GetData2():
    ...
    def train_dataloader(self):
        train_dataset = data_transform(root='../cityscapes', split='train', mode='fine', target_type='semantic', transforms=transform)
        return DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True)

    def val_dataloader(self):
        val_dataset = data_transform(root='../cityscapes', split='val', mode='fine', target_type='semantic', transforms=transform)
